...
class Calibrate:
    ...
    def calib_stack(self, kcount):
      ...
      if ret_f == True:
        ...
        img_name = 't' + str(kcount) + '_' + str(self.count) + '.jpg'
    ...
    def calibration(self):
      ...
      R1, R2, P1, P2, Q, _, _ = cv2.stereoRectify(mtx, dist, mtx, dist, self.gray.shape[::-1], R, T)
      ...
      for i in range(3):
        for j in range(3):
          ...
          cam_info = cam_info + R1[i][j] + ', ' 
          
          if i == 2 and j == 2: 
            cam_info = cam_info + ']\n'

      cam_info = cam_info + 'projection_matrix: \n rows: 3 \n  cols: 4 \n data: '
      ...
      for i in range(3):
        for j in range(4):
          ...
          if j == 3:
            cam_info = cam_info + tvec[i] + ', ' 

          else:
            cam_info = cam_info + rmat[i][j] + ', ' 
          
          if i == 2 and j == 3: 
            cam_info = cam_info + ']\n'

      file = open("~/.ros/camera_info/camera.yaml", "rw")
      file.write(cam_info)
      file.close()
