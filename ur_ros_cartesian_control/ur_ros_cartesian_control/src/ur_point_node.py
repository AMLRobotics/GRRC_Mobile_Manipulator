...
class UR():
    def __init__(self):
        ...
        self.grip_publisher = rospy.Publisher("/RobotiqVacuumGrippersRobotOutput", RobotiqVacuumGrippers_robot_output, queue_size=10)
    ...
    def gripper_control(self, gripper_on):
        if gripper_on == 'O':
            self.gripper_cmd.rACT = 1
            self.gripper_cmd.rMOD = 0
            self.gripper_cmd.rGTO = 1
            self.gripper_cmd.rATR = 0
            self.gripper_cmd.rPR = 0
            self.gripper_cmd.rSP = 150
            self.gripper_cmd.rFR = 200
            ...
            self.gripper_cmd.rGTO = 1
            self.gripper_cmd.rATR = 0
            self.gripper_cmd.rPR = 255
            self.gripper_cmd.rSP = 150
            self.gripper_cmd.rFR = 200

        self.grip_publisher.publish(self.gripper_cmd)
        rospy.sleep(0.5)
    ...
    def move_callback(self, pos_data):
        ...
        if self.move_flag == False:
            for i in range(len(move_pos)):
                self.move([move_pos[i]])
                self.gripper_control(grip_dir[i][0])
                #rospy.sleep(0.6)
    ...
    def manual_move(self, key):
        ...
        """if key == keyboard.KeyCode(char='y'):
            ...
            # pose list
            pose_list = [[-0.5, 0.0, 0.5, 180.0, 0.0, 0.0],
                    [-0.1, -0.45, 0.5, 180.0, 0.0, 90.0]]

            pose_list2 = [[-0.1, -0.45, 0.242, 180.0, 0.0, 90.0]]

            pose_list3 = [[-0.1, -0.45, 0.5, 180.0, 0.0, 90.0],
                    [-0.5, 0.0, 0.5, 180.0, 0.0, 0.0]]
            ...
            # Call Cartesian Trajectory Function with Changed Pose Parameters
            self.cartesian_traj(pose_list2, time_from_start)
            self.move_flag = False
            ...
            # Call Cartesian Trajectory Function with Changed Pose Parameters
            self.cartesian_traj(pose_list3, time_from_start)
            self.move_flag = False
        ...
        if key == keyboard.KeyCode(char='t'):
            ...
            # pose list
            pose_list = [[-0.5, 0.0, 0.5, 180.0, 0.0, 0.0],
                    [0.0, -0.45, 0.5, 180.0, 0.0, 90.0]]
            
            pose_list2 = [[0.0, -0.45, 0.242, 180.0, 0.0, 90.0]]

            pose_list3 = [[0.0, -0.45, 0.5, 180.0, 0.0, 90.0],
                    [-0.5, 0.0, 0.5, 180.0, 0.0, 0.0]]
    ...
    def cartesian_traj(self, pose_list, time_from_start=0.01, gripper = [['X']]):     
